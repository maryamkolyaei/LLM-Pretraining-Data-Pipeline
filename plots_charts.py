...
def main() -> None:
    ...
    # Drop reasons bar chart (all dropped)
    if "drop_reason" in df_dropped_all.columns:
        save_bar_counts(
            df_dropped_all["drop_reason"],
            title="Drop Reasons (all stages)",
            xlabel="drop_reason",
            out_path=PLOTS_DIR / "bar_drop_reasons.png",
        )
    ...
    # -----------------------------------------------------------------
    if "is_dup_exact" in df_dropped_v5.columns:
        save_bar_counts(
            df_dropped_v5["is_dup_exact"],
            title="Exact Duplicate Flags (Stage 5)",
            xlabel="is_dup_exact",
            out_path=PLOTS_DIR / "bar_exact_duplicates.png",
        )

    if "is_dup_near" in df_dropped_v5.columns:
        save_bar_counts(
            df_dropped_v5["is_dup_near"],
            title="Near Duplicate Flags (Stage 5)",
            xlabel="is_dup_near",
            out_path=PLOTS_DIR / "bar_near_duplicates.png",
        )
